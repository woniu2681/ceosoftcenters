...
public class DepartmentListXML {
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < 2; i++) {
			...
			Document doc = builder.build(new FileInputStream("src/boomi/gdw/DepartmentList" + i + ".xml"));

			Element root = doc.getRootElement();
			List<Element> elements = root.getChildren("Department");

			if (elements != null && !elements.isEmpty()) {
				for (Element element : elements) {
					String code = element.getChildText("externalCode");
					String date = element.getChildText("startDate");
